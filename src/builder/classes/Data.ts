    ...
    constructor(
        ...
        if(!valid && parent instanceof Data) {
            ...
            if(this instanceof ListItemData) {
                parent.__data[0][propName] = defaultData;
                this.__data = parent.__data[0][propName];
            } else {
        ...
        if(!this.__isInfo) {
            ...
            if(this.__propsList.length) {
                ...
                this.__propsList.forEach(key => {
                    if(list) this.__parsed[0][key] = (this[key] as Data).__parsed;
    ...
    public __edit(
        ...
        if(
            ...
            (// this condition check if value has changed
                ...
                (
                    !(this.__props instanceof Field && this.__props.__type === FieldTypes.Model && !style) && 
                    !lodash.isEqual(value, this.__isInfo ? this.__data : this.__data[0])
                )
            )
            ) {
            const context = window.__builder_context;
                ...
                redo: (firstTime) => {
                    ...
                    if(style) {
                        ...
                        if(this.__props instanceof Field && this.__props.__type === FieldTypes.Model){
                            ...
                            if(this instanceof ListItemData) {
                                (this.__parent as Data).__data[0][propName] = {...value, __style: this.__data.__style};
                                (this.__parent as Data).__parsed[0][propName] = rdata;
                            } else {
                            ...
                            this.__parsed[0] = this.__rdata[0];
                        }
                    }
...
export class ListData extends Data {
    ...
    constructor(
        ...
        this.__layer = null;
        this.length = this.__data[0].length as number;
    }
    ...
    private _add(data: ListItemData, index: number, affectData: boolean = true) {
        ...
        this.__parsed[0].splice(index, 0, data.__parsed);

        if(affectData) this.__data[0].splice(index, 0, data.__data);
    }
    ...
    private _delete(index: number, affectData: boolean = true) {
        ...
        this.__parsed[0].splice(index, 1);

        if(affectData) this.__data[0].splice(index, 1);
    }
